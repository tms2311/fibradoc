...
app.delete("/caixas/:id", async (req, res) => {
  try {
    ...
    // apaga anexos vinculados (se tiver)
...
app.delete("/cabos/:id", async (req, res) => {
  try {
    ...
    // bloquear se existir fusão usando esse cabo (mais seguro)
    ...
    if (f.rows[0].n > 0) {
      return res.status(400).json({
        error: "Não posso excluir: este cabo está sendo usado em fusões. Apague/ajuste as fusões antes."
      });
    }

    // apaga trechos do cabo
